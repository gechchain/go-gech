...
import (
	...
	"github.com/chain/go-gech/internal/jsre"
	"github.com/chain/go-gech/internal/jsre/deps"
	"github.com/chain/go-gech/internal/web3ext"
...
var (
	...
	wellCome       = `
		...
		try {
			message += "coinbase: " + {{.Var}}.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + {{.Var}}.blockNumber + " (" + new Date(1000 * {{.Var}}.getBlock({{.Var}}.blockNumber).timestamp) + ")\n";
		try {
...
func doWitchPrefix(js string) string {
	var buf bytes.Buffer
	type tep struct {
		Var string
		Sym string
	}
	t, _ := template.New("web3").Parse(js)
	t.Execute(&buf, &tep{Var: glob.AddrPrefix, Sym: glob.Symbol})
	return buf.String()
}

func (c *Console) initWeb3(bridge *bridge) error {
	bnJS := string(deps.MustAsset("bignumber.js"))
	web3JS := doWitchPrefix(string(deps.MustAsset("web3.js")))
...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{glob.AddrPrefix: {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
func (c *Console) Welcome() {
	...
	if res, err := c.jsre.Run(doWitchPrefix(wellCome)); err == nil {
		message += res.String()
	}
