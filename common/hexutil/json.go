...
func (b Bytes) MarshalText() ([]byte, error) {
	var result []byte
	if len(b) == 20 { //like addr
		result = make([]byte, len(b)*2+len(glob.AddrPrefix))
		copy(result, glob.AddrPrefix)
		hex.Encode(result[len(glob.AddrPrefix):], b)
	} else { //is hash
		result = make([]byte, len(b)*2+len(glob.Hash))
		copy(result, glob.Hash)
		hex.Encode(result[len(glob.Hash):], b)
	}
	//log.Info(fmt.Sprintf("%d,[%s]", len(b), result))
	return result, nil
}
...
func (b Uint64) MarshalText() ([]byte, error) {
	buf := make([]byte, len(glob.Hash), 10)
	copy(buf, glob.Hash)
...
func checkText(input []byte, wantPrefix bool) ([]byte, error) {
	...
	i, e := glob.GetHash(input, wantPrefix)
	input = i
	if e != nil {
		log.Warn(string(input), "err", ErrMissingPrefix)
...
func checkNumberText(input []byte) (raw []byte, err error) {
	...
	input, _ = glob.GetHash(input, true)
	if len(input) == 0 {
