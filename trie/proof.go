...
func VerifyProof(rootHash common.Hash, key []byte, proofDb db.KeyValueReader) (value []byte, err error) {
	key = keybytesToHex(key)
	wantHash := rootHash
	for i := 0; ; i++ {
...
func proofToPath(rootHash common.Hash, root node, key []byte, proofDb db.KeyValueReader, allowNonExistent bool) (node, error) {
...
func VerifyRangeProof(rootHash common.Hash, firstKey []byte, keys [][]byte, values [][]byte, firstProof db.KeyValueReader, lastProof db.KeyValueReader) error {
