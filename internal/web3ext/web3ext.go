...
func getEthJS() string {
	type tep struct {
		Sym string
	}
	var buf bytes.Buffer

	t, _ := template.New("").Parse(EthJs)
	t.Execute(&buf, &tep{
		Sym: glob.Symbol,
	})
	return buf.String()
...
web3._extend({
	...
	methods: [
		...
		new web3._extend.Method({
			...
			call: '{{.Sym}}_submitTransaction',
			params: 1,
			inputFormatter: [web3._extend.formatters.inputTransactionFormatter]
		}),
		new web3._extend.Method({
			name: 'fillTransaction',
			call: '{{.Sym}}_fillTransaction',
			params: 1,
			inputFormatter: [web3._extend.formatters.inputTransactionFormatter]
		}),
		new web3._extend.Method({
			name: 'getHeaderByNumber',
			call: '{{.Sym}}_getHeaderByNumber',
			params: 1
		}),
		new web3._extend.Method({
			name: 'getHeaderByHash',
			call: '{{.Sym}}_getHeaderByHash',
			params: 1
		}),
		new web3._extend.Method({
			name: 'getBlockByNumber',
			call: '{{.Sym}}_getBlockByNumber',
			params: 2
		}),
		new web3._extend.Method({
			name: 'getBlockByHash',
			call: '{{.Sym}}_getBlockByHash',
			params: 2
		}),
		new web3._extend.Method({
			name: 'getRawTransaction',
			call: '{{.Sym}}_getRawTransactionByHash',
			params: 1
		}),
		new web3._extend.Method({
			name: 'getRawTransactionFromBlock',
			call: function(args) {
				return (web3._extend.utils.isString(args[0]) && args[0].indexOf('0x') === 0) ? '{{.Sym}}_getRawTransactionByBlockHashAndIndex' : '{{.Sym}}_getRawTransactionByBlockNumberAndIndex';
			},
			params: 2,
	...
	properties: [
		new web3._extend.Property({
			...
			getter: '{{.Sym}}_pendingTransactions',
