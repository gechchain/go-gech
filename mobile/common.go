...
func (h *Hash) SetHex(hash string) error {
	...
	if strings.HasPrefix(strings.ToUpper(hash), strings.ToUpper(glob.AddrPrefix)) {
		hash = hash[len(glob.AddrPrefix):]
	}
...
func (a *Address) SetHex(address string) error {
	...
	if strings.HasPrefix(strings.ToUpper(address), strings.ToUpper(glob.AddrPrefix)) {
		address = address[len(glob.AddrPrefix):]
	}
