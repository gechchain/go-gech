...
func InitDatabaseFromFreezer(db db.Database) {
...
func iterateTransactions(db db.Database, from uint64, to uint64, reverse bool) (chan *blockTxHashes, chan struct{}) {
...
func IndexTransactions(db db.Database, from uint64, to uint64) {
...
func UnindexTransactions(db db.Database, from uint64, to uint64) {
	// short circuit for invalid range
	if from >= to {
		return
	}
