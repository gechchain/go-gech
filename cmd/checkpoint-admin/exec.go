...
import (
	...
	"github.com/chain/go-gech/contracts/checkpointoracle"
...
func publish(ctx *cli.Context) error {
	...
	for _, sig := range strings.Split(ctx.String(signaturesFlag.Name), ",") {
		trimmed := strings.TrimPrefix(strings.TrimSpace(sig), glob.AddrPrefix)
		if len(trimmed) != 130 {
	...
	head, err := client.NewClient(client).HeaderByNumber(reqCtx, nil)
	if err != nil {
		return err
	}
	num := head.Number.Uint64()
	recent, err := client.NewClient(client).HeaderByNumber(reqCtx, big.NewInt(int64(num-128)))
	if err != nil {
		return err
	}
