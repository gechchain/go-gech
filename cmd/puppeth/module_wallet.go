...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	...
	echo $'exec gech --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --gechstats \'{{.Ethstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh
