...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"gechstats", "explorer", "wallet", "faucet"} {
	...
	// If we have gechstats running, ask whether to make the secret public or not
	if w.conf.ethstats != "" {
		fmt.Println()
		fmt.Println("Include gechstats secret on dashboard (y/n)? (default = yes)")
