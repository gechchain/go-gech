...
// ethstatsDockerfile is the Dockerfile required to build an gechstats backend
// and associated monitoring site.
var ethstatsDockerfile = `
FROM puppeth/gechstats:latest

RUN echo 'module.exports = {trusted: [{{.Trusted}}], banned: [{{.Banned}}], reserved: ["yournode"]};' > lib/utils/config.js
`

// ethstatsComposefile is the docker-compose.yml file required to deploy and
// maintain an gechstats monitoring site.
var ethstatsComposefile = `
version: '2'
services:
  gechstats:
    build: .
    image: {{.Network}}/gechstats
...
// deployEthstats deploys a new gechstats container to a remote machine via SSH,
...
func deployEthstats(client *sshClient, network string, port int, secret string, vhost string, trusted []string, banned []string, nocache bool) ([]byte, error) {
	...
	// Build and deploy the gechstats service
	if nocache {
...
// ethstatsInfos is returned from an gechstats status check to allow reporting
...
// checkEthstats does a health-check against an gechstats server to verify whether
// it's running, and if yes, gathering a collection of useful infos about it.
func checkEthstats(client *sshClient, network string) (*ethstatsInfos, error) {
	// Inspect a possible gechstats container on the host
